...
private const val CHANNEL = "neptools_reminders_v2"
...
class ReminderReceiver : BroadcastReceiver() {
    override fun onReceive(context: Context, intent: Intent) {
        ...
        if (Build.VERSION.SDK_INT >= 26) {
            try {
                nm.deleteNotificationChannel("patro_reminders")
            } catch (_: Exception) {
            }
            nm.createNotificationChannel(
                NotificationChannel(CHANNEL, "NepTools सम्झना / Reminders", NotificationManager.IMPORTANCE_HIGH)
            )
        }
        ...
        val largeIcon = try {
            BitmapFactory.decodeResource(context.resources, com.neptools.app.R.drawable.ic_notification_large)
        } catch (_: Exception) {
            null
        }
